...
#' @inheritParams get_conversion_rate
...
get_conversion_rate_month <- function(fromCUR, toCUR ,start_date = Sys.Date() - 30, end_date = Sys.Date(), retried = 0) {
  tryCatch({
    response <- GET(
    ...
    log_error(e$message)
    Sys.sleep(1 + retried ^ 2)
    get_conversion_rate_month(retried = retried + 1)
  })
  convrate_table
}
